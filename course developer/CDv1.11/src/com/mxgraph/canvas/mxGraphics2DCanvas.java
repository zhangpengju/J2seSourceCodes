/**
 * $Id: mxGraphics2DCanvas.java,v 1.1 2011/06/23 06:41:28 administrator Exp $
...
//		h=h/2;
		...
		if (g.hitClip(x - pw, y - pw, w + 2 * pw, h + 2 * pw))
		{
			...
			{
				drawRoundRect(x, y, w, h, fillColor, fillPaint, penColor, shadow, true);
			}
...
//					}
	...
	/**
	 * Draws a round rectangle for the given parameters.
	 * 
	 * @param x X-coordinate of the shape.
	 * @param y Y-coordinate of the shape.
	 * @param w Width of the shape.
	 ...
	 * @param rounded Boolean indicating if the rectangle is rounded.
	 */
	protected void drawRoundRect(int x, int y, int w, int h, Color fillColor,
			Paint fillPaint, Color penColor, boolean shadow, boolean rounded)
	{
		//h*=4/5;
		int radius = (rounded) ? getArcSize(w, h) : 0;
			...
			Paint fillPaint, Color penColor, boolean isShadow)
	{
		...
		if (fillColor != null || fillPaint != null)
		{  
			Area area = new Area(new Rectangle(x+w4/2, y, w-w4, h));
			area.add(new Area(new Rectangle(x+w4/2, y, w-w4, h)));
			area.add(new Area(new Ellipse2D.Double(x, y, w4, h)));
			area.add(new Area(new Ellipse2D.Double(x+w-w4, y , w4, h)));
	 ...
	 */
	 protected void drawCloud(int x, int y, int w, int h, Color fillColor,
				Paint fillPaint, Color penColor, boolean Shadow)
		{
			...
			if (fillColor != null || fillPaint != null)
			{  
				Area area = new Area(new Rectangle(x, y, w, h-h/8));
				//area.add(new Area(new Rectangle(x+w4/2, y, w-w4, h)));
				area.add(new Area(new Ellipse2D.Double(x, y+h-h/4, w/2, h/4)));
				area.subtract(new Area(new Ellipse2D.Double(x+w/2, y+h-h/4, w/2, h/4)));
			...
			if (penColor != null)
			{
				...
				g.drawLine(x, y, x, y+h-h/8);
				g.drawLine(x+w, y, x+w, y+h-h/8);
				g.drawArc(x, y+h-h/4, w/2, h/4, 180, 180);
	            g.drawArc(x+w/2, y+h-h/4, w/2, h/4, 0, 180);
			}
		}
//	 protected void drawCloud(int x, int y, int w, int h, Color fillColor,
//			Paint fillPaint, Color penColor, boolean Shadow)
...
//		if (fillColor != null || fillPaint != null)
//		{  
//			Area area = new Area(new Rectangle(x+w4/2, y, w-w2, h));
//			area.add(new Area(new Rectangle(x+w4/2, y, w-w4, h)));
//			area.add(new Area(new Ellipse2D.Double(x, y, w4, h)));
//			area.subtract(new Area(new Ellipse2D.Double(x+w-w2, y , w4, h)));
//
//			if (Shadow)
//			{
//				g.setColor(mxConstants.SHADOW_COLOR);
...
//			g.setColor(penColor);
//
//            g.drawArc(x+w-w2, y, w4, h, 90, 180);
//			g.drawLine(x+w2, y , x+w, y );
//			g.drawLine(x + w2, y + h, x + w, y + h);
//			g.drawArc(x, y, w4, h, 90, 180);
//		}
//	}
	...
	protected void drawConnector(List<mxPoint> pts, float penWidth,
			...
			Object endMarker, float endSize, boolean dashed, boolean rounded)
	{
	...
	public void drawLine(List<mxPoint> pts, Map<String, Object> style)
	{
		...
		if (penColor != null && penWidth > 0)
		{
			...
			else
			{
				...
				float startSize = (mxUtils.getFloat(style,
						mxConstants.STYLE_STARTSIZE,
						mxConstants.DEFAULT_MARKERSIZE));
				float endSize = (mxUtils.getFloat(style,
