/*
 * $Id: EditorActions.java,v 1.1 2011/06/23 06:41:20 administrator Exp $
...
public class EditorActions
{
	...
	public static class ToggleRulersItem extends JCheckBoxMenuItem
	{
		 ...
		 * 
		 */
		public ToggleRulersItem(final CDSoftWare editor, String name)
		{
			super(name);
			setSelected(editor.getBasicGraphEditor().getGraphComponent().getColumnHeader() != null);
			...
			addActionListener(new ActionListener()
			{
				...
				{
					mxGraphComponent graphComponent = editor.getBasicGraphEditor()
							.getGraphComponent();
	...
	public static class ToggleGridItem extends JCheckBoxMenuItem
	{
		 ...
		 * 
		 */
		public ToggleGridItem(final CDSoftWare editor, String name)
		{
			super(name);
			setSelected(true);
	...
	public static class ToggleOutlineItem extends JCheckBoxMenuItem
	{
		 ...
		 * 
		 */
		public ToggleOutlineItem(final CDSoftWare editor, String name)
		{
			super(name);
			setSelected(true);
			...
			addActionListener(new ActionListener()
			{
				...
				public void actionPerformed(ActionEvent e)
				{
					final mxGraphOutline outline = editor.getBasicGraphEditor().getGraphOutline();
							...
							if (outline.getParent() instanceof JSplitPane)
							{
								if (outline.isVisible())
								{
									((JSplitPane) outline.getParent())
											.setDividerLocation(editor.getBasicGraphEditor()
													.getHeight() - 300);
									((JSplitPane) outline.getParent())
											.setDividerSize(6);
								}
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			if (editor != null)
			{
				...
				if (showDialog || editor.getCurrentFile() == null)
				{
					...
					FileFilter defaultFilter = new DefaultFileFilter(".KCM",
				...
				try
				{
					...
					else if (ext.equalsIgnoreCase("KCM")
	 ...
	 *菜单条目：保存工程事件
	 */
	public static class SaveCDAction extends AbstractAction
	{
		...
		public SaveCDAction(boolean showDialog)
		{
			this.showDialog = showDialog;
		}
		public void actionPerformed(ActionEvent e) {
			...
			if (editor != null)
			{
				...
				if (showDialog || editor.getCurrentFile() == null)
				{
					...
					// Adds the default file format
					FileFilter defaultFilter = new DefaultFileFilter(".object",
							"mxGraph Editor " + mxResources.get("file")
									+ " (.object)");
					fc.addChoosableFileFilter(defaultFilter);
				...
				try
				{
					String ext = filename
					 ...
					 if (ext.equalsIgnoreCase("object"))
					{
						mxCodec codec = new mxCodec();
						String xml = mxUtils.getXml(codec.encode(editor));

						mxUtils.writeFile(xml, filename);

						editor.setCDmodified(false);
						editor.setCurrentFile(new File(filename));
					}
	...
	public static class ToggleCreateTargetItem extends JCheckBoxMenuItem
	{
		 ...
		 * 
		 */
		public ToggleCreateTargetItem(final CDSoftWare editor, String name)
		{
			super(name);
			setSelected(true);
	...
	public static class WarningAction extends AbstractAction
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			if (e.getSource() instanceof mxGraphComponent)
			{
				...
				{
					boolean locked=(JOptionPane.showConfirmDialog(graphComponent, mxResources.get("iflock"))
					==JOptionPane.YES_OPTION);
					if(locked){
						for (int i = 0; i < cells.length; i++)
						{
							graphComponent.setCellWarning(cells[i]);
						}
					}
	 ...
	 */
	@SuppressWarnings("serial")
	public static class UnWarningAction extends AbstractAction
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			if (e.getSource() instanceof mxGraphComponent)
			{
				...
				if (cells != null && cells.length > 0)
				{
					boolean unlocked=(JOptionPane.showConfirmDialog(graphComponent, mxResources.get("ifunlock"))
					==JOptionPane.YES_OPTION);
					if(unlocked){
						for (int i = 0; i < cells.length; i++)
						{
							graphComponent.removeCellWarning(cells[i]);
						}
					}
				}
				else
	 ...
	 */
	@SuppressWarnings("serial")
	public static class CloseGraphAction extends AbstractAction
	{
	 ...
	 */
	@SuppressWarnings("serial")
	public static class NewCDAction extends AbstractAction
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			CDSoftWare cdSoftWare = getCDEditor(e);

			if (cdSoftWare != null)
			{
				if (!cdSoftWare.isCDmodified()
						|| JOptionPane.showConfirmDialog(cdSoftWare, mxResources
								.get("loseChanges")) == JOptionPane.YES_OPTION)
				{
					JDesktopPane jDesktopPane=cdSoftWare.getDesktop();
		            jDesktopPane.removeAll();
		            //JOptionPane.showConfirmDialog(cdSoftWare, ) ;
		            JOptionPane.showMessageDialog(cdSoftWare,mxResources
							.get("clear") );
				}
			}
		}
...
//		protected CDSoftWare editor;
//		public void OpenAction(CDSoftWare editor){
//			this.editor=editor;
...
//		}
			...
			if (editor != null)
			{
						...
						if (rc == JFileChooser.APPROVE_OPTION)
						{
							...
							try{
								...
								BasicGraphEditor beditor=editor.getBasicGraphEditor();
                                                               // mxGraph graph = mxGraphActions.getGraph(e);

                                                                mxGraphComponent component=(mxGraphComponent)beditor.getGraphComponent();
								...
								codec.decode(document.getDocumentElement(),
										graph.getModel());
                                                                ...
                                                                beditor.setModified(false);
								beditor.setCurrentFile(fc.getSelectedFile());
							}
	 ...
	 */
	@SuppressWarnings("serial")
	public static class OpenCDAction extends AbstractAction{
		 ...
		 * 
		 */
		public void ClearCD(CDSoftWare cdSoftWare){
			JDesktopPane jDesktopPane=cdSoftWare.getDesktop();
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			if(editor!=null)
			{
				if (!editor.isCDmodified()
						|| JOptionPane.showConfirmDialog(editor, mxResources
								.get("loseChanges")) == JOptionPane.YES_OPTION){
					    ClearCD(editor);
                         //System.out.println("the cd exist");                      
						String wd = (lastDir != null) ? lastDir : System
								.getProperty("user.dir");
						...
						// Adds file filter for supported file format
						FileFilter defaultFilter = new DefaultFileFilter(
								".object", "mxGraph Editor "
										+ mxResources.get("file") + " (.object)");
						fc.addChoosableFileFilter(defaultFilter);
						...
						if (rc == JFileChooser.APPROVE_OPTION)
						{
							...
							Document document = mxUtils.parse(mxUtils
										.readFile(fc.getSelectedFile()
												.getAbsolutePath()));
							NodeList nodeList=document.getElementsByTagName("mxGraphModel");
							if (null!=nodeList) {
                               ...
                               mxGraph graph = component.getGraph();
                                ...
                                mxCodec mdcodec = new mxCodec();
								mdcodec.decode(nodeList.item(i),
										graph.getModel());
								 resetEditor(beditor);
								 //mdDocument=null;
								 //beditor.setCurrentFile(fc.getSelectedFile());
...
//			else {
//				System.out.println("the CD is null");
//			}
	...
	public static class BackgroundAction extends AbstractAction
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			else if(e.getSource() instanceof CDSoftWare){
				mxGraphComponent graphComponent=((CDSoftWare)e.getSource()).getBasicGraphEditor().getGraphComponent();
				Color newColor = JColorChooser.showDialog(graphComponent,
						mxResources.get("background"), null);
